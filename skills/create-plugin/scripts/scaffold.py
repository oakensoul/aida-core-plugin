#!/usr/bin/env python3
"""Plugin Scaffolding Script - Two-Phase API
...
Usage:
    ...
    python scaffold.py --get-questions --context='{"plugin_name": "my-plugin"}'

    # Phase 2: Execute scaffolding (returns JSON)
    python scaffold.py --execute --context='{"plugin_name": "my-plugin", ...}'
...
# Add to path (must be before local imports)
...
def get_questions(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if not context.get("plugin_name"):
        questions.append({
            ...
            "type": "text",
            "hint": "Use kebab-case (e.g., 'my-awesome-plugin')",
            "required": True,
            "validation": "Must start with lowercase letter, contain only lowercase letters, numbers, and hyphens (2-50 chars)",
        })
    ...
    if not context.get("description"):
        questions.append({
            "id": "description",
            "question": "Provide a short description for the plugin",
            "type": "text",
            "hint": "10-500 characters, used in plugin.json and README",
            "required": True,
        })
    ...
    if not context.get("target_directory"):
        plugin_name = context.get("plugin_name", "my-plugin")
        default_target = resolve_default_target(plugin_name)
        questions.append({
            "id": "target_directory",
            "question": "Where should the plugin be created?",
            "type": "text",
            "default": default_target,
            "hint": "Absolute or relative path to an empty or non-existent directory",
        })
    ...
    if "include_agent_stub" not in context:
        questions.append({
            ...
            "question": "Include an example agent stub?",
            "type": "boolean",
            "default": False,
        })
    ...
    if "include_skill_stub" not in context:
        questions.append({
            ...
            "question": "Include an example skill stub?",
            "type": "boolean",
            "default": False,
        })
    ...
    if "keywords" not in context:
        questions.append({
            "id": "keywords",
            "question": "Comma-separated keywords/tags for the marketplace listing",
            "type": "text",
            "default": "",
            "hint": "Optional. E.g.: 'productivity, automation, tooling'",
            "required": False,
        })

    # GitHub repo creation (only if gh available)
    if "create_github_repo" not in context and check_gh_available():
        questions.append({
            "id": "create_github_repo",
            "question": "Create a GitHub repository after scaffolding?",
            "type": "boolean",
            "default": False,
        })

    # Author info (only if not inferred)
    if not inferred.get("author_name") and not context.get("author_name"):
        questions.append({
            "id": "author_name",
    ...
    if not inferred.get("author_email") and not context.get("author_email"):
        questions.append({
            "id": "author_email",
...
def execute(context: dict[str, Any]) -> dict[str, Any]:
    ...
    # Auto-convert to kebab-case if needed
    plugin_name = to_kebab_case(plugin_name)

    is_valid, error = validate_name(plugin_name)
    if not is_valid:
        return {"success": False, "message": f"Invalid plugin name: {error}"}
    ...
    try:
        license_text = get_license_text(license_id, year, author_name)
    ...
    variables: dict[str, Any] = {
        ...
        "agent_stub_name": context.get("agent_stub_name", plugin_name),
        "agent_stub_description": context.get(
            "agent_stub_description",
            f"Agent for {plugin_display_name}",
        ),
        "include_skill_stub": context.get("include_skill_stub", False),
        "skill_stub_name": context.get("skill_stub_name", plugin_name),
    ...
    try:
        ...
        # Render shared files
        all_files.extend(render_shared_files(target, variables, TEMPLATES_DIR))

        # Render language-specific files
        if language == "python":
            all_files.extend(render_python_files(target, variables, TEMPLATES_DIR))
        else:
            all_files.extend(render_typescript_files(target, variables, TEMPLATES_DIR))

        # Assemble composite files
        all_files.append(assemble_gitignore(target, language, TEMPLATES_DIR))
        all_files.append(assemble_makefile(target, language, variables, TEMPLATES_DIR))
    ...
    return {
        "success": True,
        "message": f"Plugin '{plugin_name}' scaffolded successfully",
        "path": str(target),
        "files_created": sorted(all_files),
        "language": language,
        "git_initialized": git_initialized,
        "git_committed": git_committed,
        "create_github_repo": context.get("create_github_repo", False),
        "next_steps": _build_next_steps(
            plugin_name, target, language,
...
def _build_next_steps(
    ...
    if create_repo:
        steps.append(
            f"gh repo create {plugin_name} --public --source=. --push"
        )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Plugin Scaffolding - Two-Phase API"
    )
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Phase 2: Execute scaffolding with provided context (outputs JSON)"
    )

    parser.add_argument(
        "--context",
        type=str,
        help="JSON string containing scaffolding context"
    )
